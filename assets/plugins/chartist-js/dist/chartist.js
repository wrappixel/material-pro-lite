(function (root, factory) {
  ...
  (function (window, document, Chartist) {
    ...
    Chartist.normalizeData = function (data) {
      ...
      if (data.series.length > 0 && data.labels.length === 0) {
        ...
        if (
          normalized.every(function (value) {
          ...
          labelCount = Math.max.apply(
            null,
            normalized.map(function (series) {
  ...
  (function (window, document, Chartist) {
    ...
    function AutoScaleAxis(axisUnit, data, chartRect, options) {
      ...
      var highLow =
        options.highLow ||
        Chartist.getHighLow(data.normalized, options, axisUnit.pos);
  ...
  (function (window, document, Chartist) {
    ...
    function StepAxis(axisUnit, data, chartRect, options) {
      ...
      this.stepLength =
        this.axisLength / (options.ticks.length - (options.stretch ? 1 : 0));
    }
    ...
    function createChart(options) {
      ...
      if (options.axisY.type === undefined) {
        axisY = new Chartist.AutoScaleAxis(
          ...
          Chartist.extend({}, options.axisY, {
            high: Chartist.isNum(options.high)
              ? options.high
              : options.axisY.high,
      ...
      data.raw.series.forEach(
        function (series, seriesIndex) {
          ...
          data.normalized[seriesIndex].forEach(
      ...
      data.raw.series.forEach(
        function (series, seriesIndex) {
          ...
          if (options.distributeSeries && !options.stackBars) {
            ...
            periodHalfLength =
              labelAxis.axisLength / data.normalized.length / 2;
            ...
            periodHalfLength =
              labelAxis.axisLength / data.normalized[seriesIndex].length / 2;
          }
  ...
  (function (window, document, Chartist) {
    ...
    function createChart(options) {
      ...
      var hasSingleValInSeries =
        this.data.series.filter(function (val) {
      ...
      for (var i = 0; i < this.data.series.length; i++) {
        ...
        seriesGroups[i].addClass(
          [
            options.classNames.series,
            series.className ||
              options.classNames.series + "-" + Chartist.alphaNumerate(i),
          ].join(" ")
        );
        ...
        var pathElement = seriesGroups[i].elem(
          "path",
          {
            d: path.stringify(),
          },
        ...
        if (options.showLabel) {
          ...
          var labelPosition = Chartist.polarToCartesian(
            ...
            interpolatedValue = options.labelInterpolationFnc(
              this.data.labels && !Chartist.isFalseyButZero(this.data.labels[i])
                ? this.data.labels[i]
                : dataArray[i],
              i
            );
